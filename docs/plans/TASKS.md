...
| Phase 4 | Layout والـ Navigation | `[x]` |
...
- [x] **4.1** تحديث `asideH.blade.php`
  - [x] 4.1.1 حذف الكتلة المُعلَّقة الضخمة (template demo)
  - [x] 4.1.2 استبدال الأقسام بأقسام مجال التأمين (الزيارات، الموظفون والتابعون، البيانات الأساسية، طلبات الاستبيان، إدارة المستخدمين، الإعدادات)
- [x] **4.2** تحديث `navH.blade.php` — حذف الكتل المُعلَّقة الميتة (بحث، مبدّل أنماط، روابط سريعة، إشعارات)
- [x] **4.3** التحقق من تمرير `title` بكل صفحة جديدة لاحقاً (ليست خطوة كود الآن، ملاحظة للمراحل اللاحقة)
- [x] **4.4** تنظيف نسخة الـ Vertical غير المفعّلة (`aside.blade.php`/`nav.blade.php`) — نفس روابط 4.1/4.2 بالضبط، الملفات نفسها **لا تُحذف** (تبقى لإمكانية تفعيل الوضع الجانبي لاحقاً عبر `dirNav`)
...
**ملاحظة تنفيذ:** التنفيذ تم عبر Codex (مفوَّض ومراجَع بالكامل من Claude) — تحديث `asideH.blade.php`/`navH.blade.php` (النسخة الأفقية الفعّالة حالياً) و`aside.blade.php` (النسخة الشجرية غير المفعّلة، بنفس الروابط والشروط لكن مُهيكلة حسب `menu-header` بدل `menu-toggle` متداخل — تكيّف صحيح مع اختلاف بنية الملف)، مع تأكيد أن `nav.blade.php` لم يكن يحتاج أي تعديل (لا مرجعية قديمة فيه أصلاً). تم التأكد يدوياً بمراجعة Claude من: خلو الملفات الأربعة تماماً من أي إشارة للمجال القديم (org-structure/directory/centers/... إلخ، صفر نتائج)، توازن `@if`/`@endif` و`@can`/`@endcan` بكل ملف، عدم وجود أي تلف encoding بالنص العربي الجديد (تم فحصه حرفياً حرف-حرف بعد اكتشاف مشكلة مشابهة بـ Phase 2 — نظيف هذه المرة)، وأن `routes/dashboard.php` لم يُمس (الروابط الجديدة تُشير لـ routes لسا مش موجودة، ستُبنى بـ Phase 5، وهذا متوقّع ومقصود). **ملاحظة صغيرة:** Codex صحّح تلقائياً خطأ إملائي/encoding قديم موجود أصلاً بالملفين (كلمة "الرئيسية" كانت مكتوبة بحرف hamza مركّب خاطئ `ي + ٔ` بدل الحرف الصحيح `ئ`) أثناء تعديله لسطر مجاور — تصحيح إيجابي غير مطلوب صراحة لكن غير ضار، تُرك كما هو. **الاختبار الحي (تسجيل دخول admin/receptionist ومعاينة القائمة بالمتصفح) لم يُنفَّذ بهذا الـ commit** لنفس سبب المراحل السابقة (`composer install` غير مكتمل محلياً).

**تحديث — مراجعة حية شاملة لكل المراحل 1-4 (بعد حل مشكلة composer):**

بعد ما المستخدم حل مشكلة `composer install` محلياً، تم تنفيذ فحص حي كامل (migrate:fresh --seed، تسجيل دخول فعلي admin وreceptionist عبر HTTP، معاينة كل route بالقائمة الجانبية) لأول مرة منذ بداية المشروع. النتيجة: بنية المراحل الأربعة (migrations, models, policies, Gate::before, nav) **صحيحة تماماً وتطابق التخطيط بدون أي انحراف** — لكن تم اكتشاف 3 أخطاء حقيقية منعت تشغيل التطبيق فعلياً، تم تصحيحها:

1. **`HomeController::index()` كان يشير لـ `App\Models\Project` و`App\Models\MonitoringActivity`** (موديلات من المجال القديم محذوفة فعلياً من الكود) — يسبب 500 فوري لأي مستخدم عند فتح `/`. الملف كان محمياً صراحة من الحذف بخطة Phase 1 ("يُعاد كتابتهم بالكامل بمراحل لاحقة") لكن لم يكن متوقعاً أنه سيمنع حتى تحميل الصفحة الأساسية. تم استبداله بنسخة بسيطة (ترحيب + روابط إدارية فقط) لحين البناء الفعلي لاحقاً — **هذا ليس البناء النهائي المطلوب من Phase 5/6، فقط حد أدنى يجعل التطبيق قابلاً للتشغيل والاختبار الآن.** نفس الشيء لـ `resources/views/dashboard/index.blade.php` (كانت تشير بالكامل لمحتوى المجال القديم: `dashboard.projects.*`, `dashboard.people.*`, إلخ).
2. **`VisitPolicy::view()` كان يتطلب `Visit $visit` إجبارياً** — بينما كل فحوصات القائمة الجانبية تستدعيه بصيغة class-string (`@can('view', 'App\Models\Visit')`) بدون instance، وهذا يستدعي `Gate` أن يستدعي `view($user)` بدون وسيط ثانٍ. أي مستخدم غير `super_admin` كان يتلقى `ArgumentCountError` فوري (مش مرتبط بالصلاحية نفسها، بل بتوقيع الدالة). لم يظهر هذا سابقاً لأن `super_admin` يتجاوز `Gate::before` قبل ما تُستدعى الـ Policy أصلاً. تم تصحيحه لـ `view(User $user, ?Visit $visit = null)`.
3. **`database/seeders/DatabaseSeeder.php` كان ناقص `use App\Models\User;`** (خطأ موجود مسبقاً قبل أي مرحلة من هذه الأربعة) — يمنع `php artisan db:seed` من إنشاء أي مستخدم أصلاً، فكانت قاعدة البيانات فارغة تماماً بدون هذا التصحيح. تم إضافة الـ import الناقص.

بعد التصحيحات الثلاثة: تسجيل دخول admin (`super_admin=1`) ينجح، لوحة التحكم تحمّل (200)، القائمة الجانبية تعرض كل الأقسام الجديدة بنص عربي سليم بدون أي تلف encoding. لكن روابط Phase 5 (`visits`, `employees`, `organization-units`, `medical-departments`, `survey-submissions`) لم تكن موجودة كـ routes فعلية بعد — `route()` بداخل `@can` يرمي خطأ 500 فوري (مش 404 كما كان متوقعاً بملاحظة الاختبار الأصلية أعلاه) لأي مستخدم تمر عليه شروط الـ `@can`. تم إضافة 5 routes مؤقتة بـ `routes/dashboard.php` (كل واحد closure بسيط يعرض `dashboard.pages.coming-soon.blade.php`) — **تُحذف هذه الأسطر واحداً واحداً فور ما الـ controller الحقيقي المقابل يُبنى بـ Phase 5**، هذا حل مؤقت واضح المصدر (معلَّق بتعليق `// Phase 5 placeholders`) وليس جزءاً من التصميم النهائي.

تسجيل دخول receptionist تجريبي (`role=receptionist`, `super_admin=0`, بدون أي صف `RoleUser`) اختُبر حياً أيضاً: يرى "الزيارات" فقط بالقائمة (لأن `VisitPolicy::view` يسمح للجميع بالتصميم)، ولا يرى "الموظفون"/"إدارة المستخدمين"/"البيانات الأساسية"/"الإعدادات" (لأنه بدون صلاحيات `RoleUser` صريحة) — يطابق التصميم المتوقع تماماً. حساب الاختبار حُذف بعد التحقق.

**أخطاء إضافية موجودة مسبقاً، مُوثَّقة وليست مُصلَحة (بقرار صريح من المستخدم — خارج نطاق هذه المراجعة):**
- `UserController` (create/edit/update/destroy) يوجّه لـ `dashboard.directory.*` — route group محذوف. **هذا متوقّع ومحسوم مسبقاً بخطة Phase 1** ("لا تُحذف UserController — يُعاد كتابته بالكامل بمراحل لاحقة")، فقط `index()` يعمل حالياً.
- `ConstantController` (وصفحة `constants.blade.php`) محتواها بالكامل من المجال القديم (أنواع مشاريع، مقاييس رقابة، ممولين) ويعتمد على `data/constants-registry.php` غير موجود — يسبب 500 فوري عند فتح `/constants`. **هذا لم يكن محسوماً بخطة Phase 1** (خطأ بالتدقيق الأصلي وصف `constants.blade.php` بأنه "عام ويبقى" دون تتبّع الـ Controller/الملف المطلوب فعلياً) — يحتاج قرار بمرحلة لاحقة: إما حذفه بالكامل (لا يوجد كيان "ثوابت" مذكور بمجال التأمين أصلاً بـ `01-entities.md`) أو إعادة بنائه لثوابت خاصة بالتأمين إن ظهرت حاجة فعلية.

**الخلاصة: المراحل 1-4 صحيحة بنيوياً بالكامل (migrations/models/policies/nav) وتم التحقق منها الآن حياً فعلياً وليس فقط بمراجعة استاتيكية. التطبيق قابل للتشغيل والتصفح فعلياً من الآن.** التصحيحات الثلاثة أعلاه (HomeController/index.blade.php، VisitPolicy، DatabaseSeeder import) + الـ routes المؤقتة الخمسة committed بشكل منفصل عن الـ 4 commits الأصلية للمراحل، لأنها تصحيحات اكتُشفت بمراجعة لاحقة وليست جزءاً من أي مرحلة بعينها.
